...
void print(char *string){ i32 len = strlen(string, print_maxlen); if ( len > 0 ) { write(STDOUT, string, len); } }
...
#define printf(fmt, ...) __printf(fmt, (void *[]){__VA_ARGS__, NULL}) // redefined in lib.h
    ...
    while(*c) {
        if (*c == '%' && *(c+1)) {
            ...
            c++; switch(*c){
                ...
                default: write(STDOUT, c, 1); break;
            }
            c++; last = c;
        }
