typedef struct {
    u32 entries; u32 size;
    void **item;
} list_t;

void list_print(list_t *self){
    printf("list = { %d entries\n", self->entries);
    u32 i; for(i=0; i<self->entries; i++) {
        printf("    %d: %x\n", i, self->item[i]);
    }   printf("}\n");
}

list_t *list_new(u32 size){
...
void list_remove(list_t *self, u32 index) {
    u32 i; for(i=index; i<self->entries; i++) {
